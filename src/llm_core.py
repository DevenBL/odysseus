...
@asynccontextmanager
async def _local_model_slot(
    target_url: str, model: str, workload: Optional[str] = None
):
    ...
    if kind == "foreground":
        ...
        if current.get("workload") == "background":
            ...
            if isinstance(task, asyncio.Task) and not task.done():
                logger.info(
                    "[model-gate] waiting for background local model call to finish before foreground request model=%s",
                    model,
                )
                try:
                    await task
    ...
    def check(self, text: str) -> Optional[str]:
        ...
        if not reason and len(self.recent_tokens) >= 80:
            ...
            grams = [
                ...
                for i in range(0, len(self.recent_tokens) - 3)
            ]
            if grams:
